...
sampled_stat <- pressure_data[,
    {
        id <- sample(trips_stat$tripID, test_size, replace = FALSE)
        test <- trips_stat[id]
        train <- trips_stat[-id]
        samples <- .SD[,
            {
                ...
                abuse_sample <- abuse_trips[sample(nrow(abuse_trips), size = .BY$M, replace = TRUE)]
                normal_sample <- train[sample(test_size, size = test_size - .BY$M, replace = TRUE)]
                ...
                combined <- rbindlist(list(
                    ...
                    Rt = rep(mean(train$Rt), .N),
                    Kt = rep(mean(train$Kt), .N),
