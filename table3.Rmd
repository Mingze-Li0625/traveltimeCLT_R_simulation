...
income_p_1 <- Rp_1 + Kp
income_p_0 <- Rp_0 + Kp
daily_income_t_1 <- colMeans(
    matrix(income_t_1, nrow = 10)
)
daily_income_t_0 <- colMeans(
    matrix(income_t_0, nrow = 10)
)
daily_income_p_1 <- colMeans(
    matrix(income_p_1, nrow = 10)
)
daily_income_p_0 <- colMeans(
    matrix(income_p_0, nrow = 10)
)
...
Percentage_profit <- c(
    (sum(daily_income_t_0) - sum(P)) / sum(P),
    (sum(daily_income_t_1) - sum(P)) / sum(P),
    (sum(daily_income_p_0) - sum(P)) / sum(P),
    (sum(daily_income_p_1) - sum(P)) / sum(P)
)
Average_profit <- c(
    (sum(daily_income_t_0) - sum(P)) / length(P),
    (sum(daily_income_t_1) - sum(P)) / length(P),
    (sum(daily_income_p_0) - sum(P)) / length(P),
    (sum(daily_income_p_1) - sum(P)) / length(P)
)
Maximum_Loss <- c(
    min(daily_income_t_0 - P[1:length(daily_income_t_0)]),
    min(daily_income_t_1 - P[1:length(daily_income_t_1)]),
    min(daily_income_p_0 - P[1:length(daily_income_p_0)]),
    min(daily_income_p_1 - P[1:length(daily_income_p_1)])
)
```
