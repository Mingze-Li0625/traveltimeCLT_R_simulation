...
Rt1 <- request_R(pt, user_data$start_time, user_data$start_time, user_data$distance, K = 0.9, zeta = 0.1, risk_free = 0)
Rp0 <- request_R(pp, user_data$start_time, user_data$start_time, user_data$distance, K = 0.9, zeta = 0, risk_free = 0)
Rp1 <- request_R(pp, user_data$start_time, user_data$start_time, user_data$distance, K = 0.9, zeta = 0.1, risk_free = 0)
...
Percentage_profit <- c(
    mean((Rt0 - pmax(P - Kt, 0)) / P * 100),
    mean((Rt1 - pmax(P - Kt, 0)) / P * 100),
    mean((Rp0 - pmax(P - Kp, 0)) / P * 100),
    mean((Rp1 - pmax(P - Kp, 0)) / P * 100)
)
Average_profit <- c(
    mean((Rt0 - pmax(P - Kt, 0))),
    mean((Rt1 - pmax(P - Kt, 0))),
    mean((Rp0 - pmax(P - Kp, 0))),
    mean((Rp1 - pmax(P - Kp, 0)))
)
Maximum_Loss <- c(
    min((Rt0 - pmax(P - Kt, 0))),
    min((Rt1 - pmax(P - Kt, 0))),
