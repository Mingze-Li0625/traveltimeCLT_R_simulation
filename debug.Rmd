...
```{r}
Rprof("profile.out")
s=similar_route(c(1,1,1),r=1, trips, significance=0.6, model = 'normal',Ftest_sd = F)
...
tripID = c(1,1,1)
trips[.(tripID),on = .(trip), nomatch = 0][,
  .(linkId,timebin),by = trip]
trips[data.table(trip = tripID)[, idx := .I],on = .(trip), nomatch = 0
