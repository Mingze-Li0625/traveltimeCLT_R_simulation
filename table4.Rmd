...
```{r}\
test_size <- 100 # number of trips for test set
...
Rt_0 <- na.omit(Rt_0)
Rt_0 <- mean(group_mean(Rt_0, 100 * multipler)) * 1

Kt <- mean(group_mean(Kt, 100 * multipler)) * 1
profit <- sampled_data[, .(
    profit = Rt_0 * 40 + sum(pmin(Kt - real_price, 0)),
    expand = sum((real_price))
), by = .(M, quantile, rider)]
cost_stats_table <- profit[, .(
    mean = mean((profit) / expand * test_size, na.rm = TRUE),
